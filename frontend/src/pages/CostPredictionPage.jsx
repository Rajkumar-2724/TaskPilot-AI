...
const CostPredictionPage = () => {
  ...
  const runPrediction = async (projectId) => {
    try {
      const { data } = await api.post(`/predictions/cost-overrun/${projectId}`);
      setPredictions((prev) => {
  ...
  const runAllPredictions = async () => {
    ...
    for (const p of projects) {
      try {
        const { data } = await api.post(`/predictions/cost-overrun/${p._id}`);
  ...
  if (loading && !predictions.length) {
    return <div className="text-center py-5" style={{ color: '#94A3B8' }}>Loading projects...</div>;
  }
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      <motion.h2 className="fw-bold mb-4" variants={itemVariants} style={{ fontSize: "1.8rem", fontFamily: '"Plus Jakarta Sans", sans-serif' }}>
        <span className="tp-gradient-text">Cost Overrun Prediction</span>
      </motion.h2>
          ...
          <motion.div className="col-md-4" key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="tp-card p-4">
              ...
              <div className="d-flex justify-content-between my-2">
                <span style={{ color: '#94A3B8' }}>Cost Overrun Probability:</span>
                <strong style={{ color: pred.probability > 60 ? "#EF4444" : pred.probability > 30 ? "#F59E0B" : "#22C55E" }}>
                  {pred.probability}%
                </strong>
              </div>
              <div className="d-flex justify-content-between my-2">
                <span style={{ color: '#94A3B8' }}>Predicted Final Cost:</span>
                <strong style={{ color: 'var(--tp-text)' }}>₹{(pred.predictedFinalCost / 100000).toFixed(2)} Cr</strong>
              </div>
              <div className="d-flex justify-content-between my-2">
              ...
              <div className="mt-2">
                <div className="progress" style={{ height: 8 }}>
                  <div className="progress-bar" style={{ width: `${pred.probability}%`, background: pred.probability > 60 ? "#EF4444" : pred.probability > 30 ? "#F59E0B" : "#22C55E" }} />
                </div>
              </div>
      ...
      <div className="tp-card p-4">
        ...
        <div className="table-responsive">
          <table className="table table-sm">
            <thead><tr><th>Project</th><th>Original Cost</th><th>Revised Cost</th><th>Expenditure</th><th>Physical Progress</th><th>Action</th></tr></thead>
            <tbody>
              {projects.map((p) => (
                <tr key={p._id}>
                  ...
                  <td style={{ color: '#94A3B8' }}>₹{(p.expenditure/100000).toFixed(1)}Cr</td>
